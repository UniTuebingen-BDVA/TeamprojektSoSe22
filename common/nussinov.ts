...
function duplicate(matrix_a) {
  return matrix_a.map(function (arr) {
    return arr.slice();
  });
}
...
function get_score(nuss: nussinov, pos_i: number, pos_j: number): void {
  ...
  //Case 3
  const minimal_loop_length = 1;
  if (pos_j - pos_i > minimal_loop_length) {
    if (
      is_canonical(nuss.sequence.charAt(pos_i), nuss.sequence.charAt(pos_j))
    ) {
...
function backtrace(nuss: nussinov, pos_i: number, pos_j: number): void {
  const new_pos = nuss.backtrace[pos_i][pos_j][0];
  if (pos_i < pos_j) {
    if (new_pos.length == 4) {
      // DEBUG
...
export function calculate_nussinov(sequence: string): nussinov {
  const nuss = new nussinov(sequence);
